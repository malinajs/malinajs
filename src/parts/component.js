...
export function makeComponent(node, requireCD) {
  ...
  let reference = null;
  const propsFn = []; const propsSetter = []; const $class = []; let staticProps = true;
  const slotBlocks = [];
  const anchorBlocks = [];

  const componentName = node.name;
  if (componentName != 'component' && this.config.autoimport) {
    const imported = this.script.autoimport[componentName] || this.script.importedNames.includes(componentName) ||
            this.script.rootVariables[componentName] || this.script.rootFunctions[componentName];

    if (!imported) {
      const r = this.config.autoimport(componentName, this.config.path, this);
  ...
  if (node.body && node.body.length) {
    ...
    Object.values(slots).forEach((slot) => {
      ...
      let props;
      const rx = slot.value && slot.value.match(/^#slot\S*\s+(.*)$/);
      if (rx) {
      ...
      const contentNodes = trimEmptyNodes(slot.body);
      if (contentNodes.length == 1 && contentNodes[0].type == 'node' && contentNodes[0].name == 'slot') {
        const parentSlot = contentNodes[0];
      ...
      const block = this.buildBlock(slot, { inline: true });
    ...
    anchors.forEach((n) => {
      const bb = this.buildBlock({ body: [n] }, { inline: true, oneElement: 'el', bindAttributes: true });
      const block = bb.source;
      const name = n.name.slice(1) || 'default';
  ...
  propList.forEach((prop) => {
    ...
    if (name[0] == '#') {
      ...
      const { event, fn } = this.makeEventProp(prop);
      ...
      let metaClass; const args = name.split(':');
      if (args.length == 1) {
        metaClass = '$$main';
      } else {
    ...
    const ip = this.inspectProp(prop);
  ...
  const result = xNode('component', {
    ...
    if (n.reference) throw new Error('not implemented'); // FIXME
    let comma = false;
...
export function makeComponentDyn(node, requireCD, element) {
  ...
  const component = this.makeComponent(node, requireCD).bind;
