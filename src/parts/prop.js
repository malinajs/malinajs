...
export function bindProp(prop, node, element, requireCD) {
  ...
  if (name[0] == '#') {
    ...
    const { event, fn, rootModifier } = this.makeEventProp(prop, () => element.bindName());
    ...
    const argName = 'a' + (this.uniqIndex++);
    ...
    const styleName = toCamelCase(arg);
    let exp;
    if (prop.value) {
    ...
    const hasElement = exp.includes('$element');
    return {
      bind: xNode('block', {
        scope: hasElement,
    ...
    if (arg) {
      ...
      const args = prop.value ? `, () => [${getExpression()}]` : '';
    ...
    const props = node.attributes.filter((a) => a.name == 'class' || a.name.startsWith('class:'));
    ...
    if (compound) {
      const classes = Array.from(node.classes);
      node.classes.clear();
      if (this.config.passClass) this.require('resolveClass');
      const exp = props.map((prop) => {
        if (prop.name == 'class') {
          return this.parseText(prop.value).result;
        } else {
          const className = prop.name.slice(6);
          assert(className);
          const exp = prop.value ? unwrapExp(prop.value) : className;
          this.detectDependency(exp);
    ...
    if (prop.value && prop.value.indexOf('{') >= 0) {
      ...
      const hasElement = prop.value.includes('$element');
      ...
      const n = xNode('bindAttribute', {
        ...
        if (propList[name]) {
          const propName = propList[name] === true ? name : propList[name];
          if (ctx.inuse.apply) {
            requireCD.$value(true);
