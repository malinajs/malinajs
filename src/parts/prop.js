  ...
  if (name == "on") {
    ...
    opt.slice(1).forEach((opt) => {
      if (opt == "preventDefault") mod += `$event.preventDefault();`;
        ...
        mod += `if($event.keyCode != 13) return; $event.preventDefault();`;
      else if (opt == "escape")
        mod += `if($event.keyCode != 27) return; $event.preventDefault();`;
    ...
    return {
      bind: `{
            let $element=${makeEl()};
            $cd.ev($element, "${event}", ($event) => { ${mod} $$apply(); ${Q(
        exp
      )}});
            }`,
    };
    ...
    if (prop.value && prop.value.indexOf("{") >= 0) {
      ...
      if (
        ["hidden", "checked", "value", "disabled", "selected"].indexOf(name) >=
        0
      ) {
        return {
          bind: `{
