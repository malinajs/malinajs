...
export function attachHead(n, requireCD) {
  if (n.elArg == 'window' || n.elArg == 'body') {
    const name = 'el' + (this.uniqIndex++);
    const block = this.buildBlock({ body: [n] }, { malinaElement: true, inline: true, oneElement: name, bindAttributes: true });
    if (block.source) {
      return xNode('block', {
        name,
        target: n.elArg,
    ...
    if (title?.body?.[0]) {
      ...
      const r = this.parseText(title.body[0].value);
