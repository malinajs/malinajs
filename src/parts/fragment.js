...
export function makeFragment(node, requireCD) {
  const rx = node.value.match(/#fragment\:(\S+)(.*)$/);
  assert(rx);
  const name = rx[1]; let external = false;
...
export function attachFragment(node) {
  const name = node.elArg;
  assert(isSimpleName(name));

  let slot = null;
  if (node.body?.length) slot = this.buildBlock({ body: trimEmptyNodes(node.body) }, { inline: true });

  const { props, events, forwardAllEvents, staticProps } = parseAttibutes.call(this, node.attributes);
...
export function attchExportedFragment(node, label, componentName, requireCD) {
  ...
  const pa = parseAttibutes.call(this, node.attributes);
  data = { ...pa, ...data };
