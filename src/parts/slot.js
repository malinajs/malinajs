...
export function attachSlot(slotName, node, requireCD) {
  const props = []; let staticProps = true;

  if (node.attributes && node.attributes.length) {
    node.attributes.forEach((prop) => {
      const { name, value, ...ip } = this.inspectProp(prop);
  ...
  const result = xNode('slot', {
    ...
    const dynamicProps = this.glob.apply.value && !n.staticProps;

    if (dynamicProps) n.requireCD.$value(true);

    let missed = ''; const slotName = n.name == 'default' ? 'null' : n.name;
