...
export function makeEventProp(prop, requireElement) {
  ...
  let event = '';
  const modList = []; let _mod = '';
  let handler = ''; let exp; let func;
  let step = 0;
  ...
  if (exp) {
    const type = detectExpressionType(exp);
    if (type == 'identifier') {
  ...
  const keyEvent = ['keydown', 'keypress', 'keyup'].includes(event);
  const keyCodes = {
    enter: 'Enter',
    tab: 'Tab',
    esc: 'Escape',
  ...
  modList.forEach((opt) => {
    ...
    throw new Error('Wrong modificator: ' + opt);
  });
