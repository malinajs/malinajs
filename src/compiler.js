...
function detectDependency(data) {
  const check = (name) => {
    for (const k of ['$props', '$attributes', '$emit', '$context']) {
...
function loadConfig(filename, option) {
  ...
  for (let i = parts.length - 1; i > 1; i--) {
    const local = parts.slice(0, i).join('/') + '/malina.config.js';
