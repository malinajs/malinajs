...
export function processCSS() {
  const styleNodes = this.styleNodes;
  const genId = () => this.config.cssGenId ? this.config.cssGenId() : utilsGenId();

  const self = this.css = { id: genId(), externalMainName: null };
  const astList = [];
  const selectors = {};
  const removeBlocks = [];
  let active = false;
  ...
  const convertAst = (node, parent) => {
    ...
    const r = { parent };
    const newParent = node.type ? r : parent;
    for (const k in node) r[k] = convertAst(node[k], newParent);
    return r;
  };

  const parseCSS = (content, option) => {
    const ast = csstree.parse(content, option);
  ...
  function transform(styleNode) {
    ...
    const ast = parseCSS(styleNode.content);
    astList.push(ast);
    ...
    csstree.walk(ast, function(node) {
      if (node.type == 'Declaration') {
        ...
        const emptyBlock = node.block.children.length == 0;
        if (emptyBlock) removeBlocks.push(node);

        const selectorList = node.prelude.children;
        const processSelector = function(fullSelector) {
          ...
          fullSelector.children.forEach((sel) => {
            if (sel.type == 'PseudoClassSelector' && sel.name == 'global') {
              sel = sel.children[0];
              assert(sel.type == 'Raw');
              const a = parseCSS(sel.value, { context: 'selector' });
          ...
          const cleanSelector = selector2str(cleanSelectorItems);
          ...
          for (let i = hashed.length - 1; i >= 0; i--) {
            const sel = hashed[i];
            const left = hashed[i - 1];
            const right = hashed[i + 1];
  ...
  const resolveHashes = () => {
    ...
    Object.values(selectors).forEach((sel) => {
      ...
      sel.hashedSelectors.forEach((hashed) => {
        const hash = hashed._external ? sel.external : sel.local;
        assert(hash);
        hashed.forEach((n) => {
          if (!n.__hash) return;
          n.name = hash;
  ...
  self.getClassMap = () => {
    ...
    Object.values(selectors).forEach((sel) => {
      ...
      const className = sel.source ? sel.source[0].children[0].name : sel.cleanSelector.substring(1);
      if (sel.external) {
  ...
  self.process = function(data) {
    ...
    Object.values(selectors).forEach((sel) => {
      ...
      try {
        ...
        const e = new Error(`CSS error: '${selector2str(sel.source[0])}'`);
...
function makeDom(data) {
  function build(parent, list) {
    list.forEach((e) => {
      ...
      const n = new Node(e.name, { __node: e });
...
Node.prototype.getElementsByClassName = function(names) {
  names = names.split(/\s+/);
  if (names.length != 1) throw new Error('Not supported');
  const cls = names[0];

  const result = [];
  this.childNodes.forEach((n) => {
    const rx = RegExp('(^|\\s)' + cls + '(\\s|$)', 'i');
