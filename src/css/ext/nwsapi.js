...
(function Export(global, factory) {
  ...
  const unique =
    function(nodes) {
      let i = 0; let j = -1; let l = nodes.length + 1; const list = [];
      while (--l) {
    ...
    function(str) {
      return REX.HasEscapes.test(str)
        ? str.replace(REX.FixEscapes,
          function(substring, p1, p2) {
            // unescaped " or '
            return p2 || (REX.HexNumbers.test(p1) ? stringFromCodePoint(parseInt(p1, 16))
              // \' \"
              : REX.EscOrQuote.test(p1) ? substring
              // \g \h \. \# etc
                : p1);
      ...
      if (Config.IDS_DUPES === false) {
        ...
        if ('all' in context) {
          if ((e = context.all[id])) {
            ...
            for (let i = 0, l = e.length, nodes = []; l > i; ++i) {
  ...
  const nthOfType = (function() {
    ...
    return function(element, dir) {
      ...
      let e; let i; let j; let k; let l; const name = element.nodeName;
  ...
  const S_HEAD = 'var lvl=[],e,n,o,j=r.length-1,k=-1';
  ...
  const S_BODY = 'r[++j]={node:c[k],lvl:lvl};lvl=[];';
  ...
  const compile =
    function(selector, mode, callback) {
      let factory; let token; let head = ''; let loop = ''; let macro = ''; let source = ''; let vars = '';
  ...
  var compileSelector =
    function(expression, source, mode, callback, not) {
      ...
      let a; let b; let n; let f; let i; let l; let name; let nested; let NS;
      const N = not ? '!' : ''; const D = not ? '' : '!';
      let compat; let expr; let match; let result; let status; let symbol; let test;
      let type; let selector = expression; let selector_string; let vars;
            ...
            case 'focus-within':
              source = 'hasFocus' in doc
                ? 'n=s.doc.activeElement;while(e){if(e===n||e.parentNode===n)break;}' +
                    'if(' + N + '(e===n&&s.doc.hasFocus()&&(e.type||e.href||typeof e.tabIndex=="number"))){' + source + '}'
                : source;
              break;
            default:
              emit('\'' + selector_string + '\'' + qsInvalid);
      ...
      if (selectors) {
        if ((resolver = selectResolvers[selectors])) {
          if (resolver.context === context && resolver.callback === callback) {
            const f = resolver.factory; const h = resolver.htmlset; const n = resolver.nodeset; var nodes = [];
            if (n.length > 1) {
  ...
  let _closest; let _matches; let _querySelector; let _querySelectorAll;
  ...
  const install =
    function(all) {
      ...
      Element.prototype.closest =
        function closest() {
          const ctor = Object.getPrototypeOf(this).__proto__.__proto__.constructor.name;
          if (!('nodeType' in this)) { emit('\'closest\' called on an object that does not implement interface ' + ctor + '.', TypeError); }
          return arguments.length < 1
            ? ancestor.apply(this, [])
            : arguments.length < 2
              ? ancestor.apply(this, [arguments[0], this])
      ...
      DocumentFragment.prototype.querySelector =
        function querySelector() {
          ...
          return arguments.length < 1
            ? first.apply(this, [])
            : arguments.length < 2
              ? first.apply(this, [arguments[0], this])
  ...
  const Dom = {
    ...
    registerCombinator:
      function(combinator, resolver) {
        let i = 0; const l = combinator.length; let symbol;
        for (; l > i; ++i) {
          if (combinator[i] != '=') {
            symbol = combinator[i];
            break;
    ...
    registerOperator:
      function(operator, resolver) {
        let i = 0; const l = operator.length; let symbol;
        for (; l > i; ++i) {
          if (operator[i] != '=') {
            symbol = operator[i];
            break;
