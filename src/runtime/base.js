...
export const $$removeElements = (el, last) =>
  iterNodes(el, last, (n) => n.remove());
...
export const attachDynComponent = (label, exp, bind) => {
  ...
  let active,
    ...
    $cd,
    $dom,
    finalLabel = getFinalLabel(label);
...
export const setClassToElement = (element, value) =>
  bindAttributeBase(element, "class", value);

export const bindClassExp = (element, fn) => {
  $watch(fn, (value) => setClassToElement(element, value), {
    ro: true,
    value: "",
  });
};
  ...
  return (line, defaults) => {
    ...
    line
      ...
      .forEach((name) => {
        ...
        if (h) {
          let className = (
            $option.$class[name === mainName ? "$$main" : name] || ""
          ).trim();
          if (className) {
            result[className] = 1;
          } else if (h !== true) {
...
export const eachDefaultKey = (item, index, array) =>
  isObject(array[0]) ? item : index;
...
export const callExportedFragment = (
  childComponent,
  name,
  slot,
  events,
