import { svgElements, replaceElementKeyword, assert } from './utils.js';
import { xNode } from './xnode.js';


export function buildRuntime() {
  const runtime = xNode('block', { scope: true, $compile: [] });

  const rootCD = this.glob.rootCD;
  ...
  const bb = this.buildBlock(this.DOM, {
    inline: true,
    template: {
      name: '$parentElement',
      cloneNode: true
  ...
  runtime.push(xNode('addStyle', (ctx) => {
    ...
    const style = this.css.getContent();
    if (!style) return;
    const config = ctx._ctx.config;
    if (config.css) {
...
export function buildBlock(data, option = {}) {
  const rootTemplate = xNode('node', { inline: true, _ctx: this });
  let rootSVG = false; let requireFragment = option.template?.requireFragment;
  const binds = xNode('block');
  const result = {};
  const requireCD = result.requireCD = xNode('require-cd', false);
  const inuse = Object.assign({}, this.inuse);
  ...
  if (option.allowSingleBlock && data.body.length == 1) {
    ...
    if (n.type == 'node' && n.name.match(/^[A-Z]/)) {
      const component = this.makeComponent(n, requireCD);
      return {
        requireCD,
  ...
  const go = (data, isRoot, tpl) => {
    let body = data.body.filter((n) => {
      ...
      if (n.type == 'fragment') {
        try {
          const f = this.makeFragment(n, requireCD);
          f && binds.push(f);
        } catch (e) {
          wrapException(e, n);
        }
      ...
      if (n.type === 'text') {
        ...
        if (n.name == 'component' || n.name.match(/^[A-Z]/)) {
          if (n.name == 'component' || !n.elArg) {
            ...
            const el = placeLabel(`exported ${n.elArg}`);
            const b = this.attchExportedFragment(n, el, n.name, requireCD);
            b && binds.push(b);
          }
          return;
        }
        if (n.name == 'slot') {
          ...
          if (!slotName) {
            if (option.context == 'fragment') {
              const el = placeLabel('fragment-slot');
              binds.push(this.attachFragmentSlot(el, requireCD));
              return;
            } else slotName = 'default';
          }

          const el = placeLabel(slotName);
          const slot = this.attachSlot(slotName, n, requireCD);
        ...
        if (n.name == 'fragment') {
          ...
          const el = placeLabel(`fragment ${n.elArg}`);
        ...
        n.attributes.forEach((p) => {
          const b = this.bindProp(p, n, el, requireCD);
          if (b) {
        ...
        if (option.bindAttributes && (el.attributes.length || el.class.size)) {
          ...
          binds.push(xNode('bindClasses', { el }, (ctx, n) => {
            const el = n.el;
            const elName = el.bindName();
            if (el.class.size) {
              const className = Array.from(el.class.values()).join(' ');
        ...
        if (data.type == 'node' && data.body.length == 1) {
          lastStatic = null;
          const eachBlock = this.makeEachBlock(n, {
            elName: tpl.bindName(),
            onlyChild: true
          });
          ...
          if (isRoot) requireFragment = true;
          const element = placeLabel(n.value);
          const eachBlock = this.makeEachBlock(n, { elName: element.bindName() });
          binds.push(eachBlock.source);
        }
      } else if (n.type === 'if') {
        if (isRoot) requireFragment = true;
        ...
        const r = n.value.match(/^@(\w+)\s+(.*)$/s);
        const name = r[1];
        const exp = r[2];

        if (name == 'html') {
          if (isRoot) requireFragment = true;
          const el = placeLabel('html');
          binds.push(this.makeHtmlBlock(exp, el, requireCD));
        } else throw new Error('Wrong tag');
      } else if (n.type === 'await') {
        if (isRoot) requireFragment = true;
        requireCD.$value(true);
        const el = placeLabel(n.value);
        const r = this.makeAwaitBlock(n, el);
        r && binds.push(r);
      ...
      innerBlock.push(xNode('bindNodes', {
        ...
        const gen = (parent, parentName) => {
          for (let i = 0; i < parent.children.length; i++) {
            ...
            const diff = i == 0 ? '[$runtime.firstChild]' : `[$runtime.childNodes][${i}]`;
    ...
    result.block = xNode('block', {
      ...
      if (!ctx.isEmpty(n.innerBlock)) {
        if (n.each) {
          ...
          const extra = option.extraArguments ? ', ' + option.extraArguments.join(', ') : '';
